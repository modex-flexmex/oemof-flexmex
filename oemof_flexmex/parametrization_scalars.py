...
def update_electricity_shortage(component_df, scalars):
    ...
    component_df["marginal_cost"] = (
        get_parameter_values(scalars, "Energy_SlackCost_Electricity") * 1e-3
    )  # Eur/GWh to Eur/MWh
...
def update_heat_shortage(component_df, scalars):
    ...
    # Fill column 'marginal_cost' with a fixed value for ALL the elements
    component_df["marginal_cost"] = (
        get_parameter_values(scalars, "Energy_SlackCost_Heat") * 1e-3
    )  # Eur/GWh to Eur/MWh
...
def update_electricity_demand(component_df, scalars):
    ...
    component_df["amount"] = (
        get_parameter_values(scalars, "Energy_FinalEnergy_Electricity") * 1e3
    )  # GWh to MWh
...
def update_heat_demand(component_df, scalars):
    ...
    # Fill column for ALL the elements
    component_df["amount"] = (
        get_parameter_values(scalars, "Energy_FinalEnergy_Heat") * 1e3
    )  # GWh to MWh
...
def update_bpchp(component_df, scalars):
    ...
    component_df["electric_efficiency"] = (
        get_parameter_values(
    ...
    component_df["thermal_efficiency"] = (
        get_parameter_values(
            scalars, "EnergyConversion_EtaNominal_ElectricityHeat_CH4_BpCCGT"
        )
    ...
    component_df["carrier_cost"] = (
        ...
        * get_parameter_values(scalars, "Energy_EmissionFactor_CH4")
    ) * 1e-3  # Eur/GWh to Eur/MWh

    component_df["marginal_cost"] = (
        get_parameter_values(
            scalars, "EnergyConversion_VarOM_ElectricityHeat_CH4_BpCCGT"
        )
        * 1e-3
    )  # Eur/GWh to Eur/MWh
...
def update_extchp(component_df, scalars):
    ...
    component_df["condensing_efficiency"] = (
        get_parameter_values(
...
def update_boiler_large(component_df, scalars):
    ...
    component_df["efficiency"] = (
        get_parameter_values(scalars, "EnergyConversion_Eta_Heat_CH4_Large") * 0.01
    )  # Percent to decimals

    component_df["carrier_cost"] = (
        get_parameter_values(scalars, "Energy_Price_CH4") * 1e-3
    )  # Eur/GWh to Eur/MWh

    component_df["marginal_cost"] = (
        get_parameter_values(scalars, "EnergyConversion_VarOM_Heat_CH4_Large") * 1e-3
    )  # Eur/GWh to Eur/MWh
...
def update_boiler_small(component_df, scalars):
    ...
    component_df["efficiency"] = (
        get_parameter_values(scalars, "EnergyConversion_Eta_Heat_CH4_Small") * 0.01
    )  # Percent to decimals
    ...
    component_df["marginal_cost"] = (
        get_parameter_values(scalars, "EnergyConversion_VarOM_Heat_CH4_Small") * 1e-3
    )  # Eur/GWh to Eur/MWh
...
def update_pth(component_df, scalars):
    ...
    component_df["efficiency"] = (
        get_parameter_values(scalars, "EnergyConversion_Eta_Heat_Electricity_Large")
        * 0.01
    )  # Percent to decimals

    component_df["marginal_cost"] = (
        get_parameter_values(scalars, "EnergyConversion_VarOM_Heat_Electricity_Large")
        * 1e-3
    )  # Eur/GWh to Eur/MWh
...
def update_electricity_heatpump_small(component_df, scalars):
    ...
    component_df["marginal_cost"] = (
        get_parameter_values(
            scalars, "EnergyConversion_VarOM_Heat_ElectricityHeat_Small"
        )
        * 1e-3
    )  # Eur/GWh to Eur/MWh
...
def update_electricity_heatpump_large(component_df, scalars):
    ...
    component_df["marginal_cost"] = (
        get_parameter_values(
            scalars, "EnergyConversion_VarOM_Heat_ElectricityHeat_Large"
        )
        * 1e-3
    )  # Eur/GWh to Eur/MWh
...
def update_heat_storage_small(component_df, scalars):
    ...
    component_df["capacity"] = get_parameter_values(
        scalars, "Storage_Capacity_Heat_SmallCharge"
    )

    component_df["storage_capacity"] = (
        get_parameter_values(scalars, "Storage_Capacity_Heat_SmallStorage") * 1e3
    )  # GWh to MWh

    component_df["loss_rate"] = (
        get_parameter_values(scalars, "Storage_SelfDischarge_Heat_Small") * 0.01
    )  # Percent to decimals

    component_df["efficiency"] = (
        get_parameter_values(scalars, "Storage_Eta_Heat_SmallCharge") * 0.01
    )  # Percent to decimals

    component_df["marginal_cost"] = (
        get_parameter_values(scalars, "Storage_VarOM_Heat_Small") * 1e-3
    )  # Eur/GWh to Eur/MWh
...
def update_heat_storage_large(component_df, scalars):
    ...
    component_df["capacity"] = get_parameter_values(
        scalars, "Storage_Capacity_Heat_LargeCharge"
    )

    component_df["storage_capacity"] = (
        get_parameter_values(scalars, "Storage_Capacity_Heat_LargeStorage") * 1e3
    )  # GWh to MWh

    component_df["loss_rate"] = (
        get_parameter_values(scalars, "Storage_SelfDischarge_Heat_Large") * 0.01
    )  # Percent to decimals

    component_df["efficiency"] = (
        get_parameter_values(scalars, "Storage_Eta_Heat_LargeCharge") * 0.01
    )  # Percent to decimals

    component_df["marginal_cost"] = (
        get_parameter_values(scalars, "Storage_VarOM_Heat_Large") * 1e-3
    )  # Eur/GWh to Eur/MWh
...
def update_link(component_df, scalars):
    ...
    transmission_loss_per_100km = get_parameter_values(
        scalars, "Transmission_Losses_Electricity_Grid"
    )

    transmission_length = get_parameter_values(
        scalars, "Transmission_Length_Electricity_Grid"
    )

    transmission_capacity = get_parameter_values(
        scalars, "Transmission_Capacity_Electricity_Grid"
    )
    ...
    component_df["marginal_cost"] = varom * transmission_length
...
def update_wind_onshore(component_df, scalars):
    ...
    component_df["capacity"] = get_parameter_values(
        scalars, "EnergyConversion_Capacity_Electricity_Wind_Onshore"
    )
...
def update_wind_offshore(component_df, scalars):
    ...
    component_df["capacity"] = get_parameter_values(
        scalars, "EnergyConversion_Capacity_Electricity_Wind_Offshore"
    )
...
def update_solar_pv(component_df, scalars):
    ...
    component_df["capacity"] = get_parameter_values(
        scalars, "EnergyConversion_Capacity_Electricity_Solar_PV"
    )

    return component_df


def update_h2_cavern(component_df, scalars, expandable=False, greenfield=False):
    r"""
    ...
    availability = (
        get_parameter_values(scalars, "Storage_Availability_H2_Cavern") * 1e-2
    )  # percent -> 0...1

    capacity_charge = get_parameter_values(scalars, "Storage_Capacity_H2_CavernCharge")

    capacity_discharge = get_parameter_values(
        scalars, "Storage_Capacity_H2_CavernDischarge"
    )

    storage_capacity = (
        get_parameter_values(scalars, "Storage_Capacity_H2_CavernStorage") * 1e3
    )  # GWh to MWh

    self_discharge = (
        get_parameter_values(scalars, "Storage_SelfDischarge_Electricity_H2_Cavern")
        * 1e-2
    )  # percent -> 0...1

    operation_cost = (
        get_parameter_values(scalars, "Storage_VarOM_H2_Cavern") * 1e-3
    )  # Eur/GWh -> Eur/MWh

    eta_charge = (
        get_parameter_values(scalars, "Storage_Eta_H2_CavernCharge") * 1e-2
    )  # percent -> 0...1

    eta_discharge = (
        get_parameter_values(scalars, "Storage_Eta_H2_CavernDischarge") * 1e-2
    )  # percent -> 0...1

    # Investment parameters
    capex_charge = get_parameter_values(scalars, "Storage_Capex_H2_CavernCharge")

    capex_discharge = get_parameter_values(scalars, "Storage_Capex_H2_CavernDischarge")

    capex_storage = (
        get_parameter_values(scalars, "Storage_Capex_H2_CavernStorage") * 1e-3
    )  # Eur/GWh -> Eur/MWh

    fix_cost = (
        get_parameter_values(scalars, "Storage_FixOM_H2_Cavern") * 1e-2
    )  # percent -> 0...1
    ...
    lifetime = get_parameter_values(scalars, "Storage_LifeTime_H2_CavernStorage")

    interest = (
        get_parameter_values(scalars, "EnergyConversion_InterestRate_ALL") * 1e-2
    )  # percent -> 0...1

    annualized_cost_charge = annuity(capex=capex_charge, n=lifetime, wacc=interest)

    annualized_cost_discharge = annuity(
        capex=capex_discharge, n=lifetime, wacc=interest
    )

    annualized_cost_storage = annuity(capex=capex_storage, n=lifetime, wacc=interest)
    ...
    if expandable:
        component_df["capacity_cost_charge"] = (
            annualized_cost_charge + fix_cost * capex_charge
        )
        component_df["capacity_cost_discharge"] = (
            annualized_cost_discharge + fix_cost * capex_discharge
        )

        component_df["storage_capacity_cost"] = (
            annualized_cost_storage + fix_cost * capex_storage
        )
...
def update_liion_battery(component_df, scalars, expandable=False, greenfield=False):
    r"""
    ...
    # Operation parameters
    availability = (
        get_parameter_values(scalars, "Storage_Availability_Electricity_LiIonBattery")
        * 1e-2
    )  # percent -> 0...1
    ...
    storage_capacity = (
        get_parameter_values(
            scalars, "Storage_Capacity_Electricity_LiIonBatteryStorage"
        )
        * 1e3
    )  # GWh to MWh

    self_discharge = (
        get_parameter_values(scalars, "Storage_SelfDischarge_Electricity_LiIonBattery")
        * 1e-2
    )  # percent -> 0...1

    operation_cost = (
        get_parameter_values(scalars, "Storage_VarOM_Electricity_LiIonBattery") * 1e-3
    )  # Eur/GWh -> Eur/MWh

    eta_charge = (
        get_parameter_values(scalars, "Storage_Eta_Electricity_LiIonBatteryCharge")
        * 1e-2
    )  # percent -> 0...1

    eta_discharge = (
        get_parameter_values(scalars, "Storage_Eta_Electricity_LiIonBatteryDischarge")
        * 1e-2
    )  # percent -> 0...1

    # Investment parameters
    capex_charge = get_parameter_values(
        scalars, "Storage_Capex_Electricity_LiIonBatteryCharge"
    )

    capex_discharge = get_parameter_values(
        scalars, "Storage_Capex_Electricity_LiIonBatteryDischarge"
    )

    capex_storage = (
        get_parameter_values(scalars, "Storage_Capex_Electricity_LiIonBatteryStorage")
        * 1e-3
    )  # Eur/GWh -> Eur/MWh

    fix_cost = (
        get_parameter_values(scalars, "Storage_FixOM_Electricity_LiIonBattery") * 1e-2
    )  # percent -> 0...1
    ...
    lifetime = get_parameter_values(
        scalars, "Storage_LifeTime_Electricity_LiIonBatteryStorage"
    )
    ...
    annualized_cost_charge = annuity(
        capex=capex_charge + capex_discharge, n=lifetime, wacc=interest
    )
    ...
    if expandable and greenfield:
        ...
        component_df["capacity"] = (
            (capacity_charge + capacity_discharge) / 2 * availability
        )
        component_df["storage_capacity"] = storage_capacity * availability
    ...
    if expandable:
        component_df["capacity_cost"] = annualized_cost_charge + fix_cost * (
            capex_charge + capex_discharge
        )
...
def update_nuclear_st(component_df, scalars, expandable=False, greenfield=False):
    ...
    capacity = get_parameter_values(
        scalars, "EnergyConversion_Capacity_Electricity_Nuclear_ST"
    )

    availability = (
        get_parameter_values(
            scalars, "EnergyConversion_Availability_Electricity_Nuclear_ST"
        )
        * 1e-2
    )  # percent -> 0...1

    operation_cost = (
        get_parameter_values(scalars, "EnergyConversion_VarOM_Electricity_Nuclear_ST")
        * 1e-3
    )  # Eur/GWh -> Eur/MWh

    eta = (
        get_parameter_values(scalars, "EnergyConversion_EtaNet_Electricity_Nuclear_ST")
        * 1e-2
    )  # percent -> 0...1

    carrier_price = (
        get_parameter_values(scalars, "Energy_Price_Uranium") * 1e-3
    )  # Eur/GWh -> Eur/MWh

    # Investment parameters
    capex = get_parameter_values(
        scalars, "EnergyConversion_Capex_Electricity_Nuclear_ST"
    )

    fix_cost = (
        get_parameter_values(scalars, "EnergyConversion_FixOM_Electricity_Nuclear_ST")
        * 1e-2
    )  # percent -> 0...1

    lifetime = get_parameter_values(
        scalars, "EnergyConversion_LifeTime_Electricity_Nuclear_ST"
    )
    ...
    component_df["expandable"] = expandable
    component_df["capacity"] = (
        0 if expandable and greenfield else capacity * availability
    )
...
def update_ch4_gt(component_df, scalars, expandable=False, greenfield=False):
    ...
    availability = (
        get_parameter_values(
            scalars, "EnergyConversion_Availability_Electricity_CH4_GT"
        )
        * 1e-2
    )  # percent -> 0...1

    operation_cost = (
        get_parameter_values(scalars, "EnergyConversion_VarOM_Electricity_CH4_GT")
        * 1e-3
    )  # Eur/GWh -> Eur/MWh

    eta = (
        get_parameter_values(scalars, "EnergyConversion_EtaNet_Electricity_CH4_GT")
        * 1e-2
    )  # percent -> 0...1
    ...
    emission_factor = (
        get_parameter_values(scalars, "Energy_EmissionFactor_CH4") * 1e-3
    )  # t/GWh -> t/MWh

    # Investment parameters
    capex = get_parameter_values(scalars, "EnergyConversion_Capex_Electricity_CH4_GT")

    fix_cost = (
        get_parameter_values(scalars, "EnergyConversion_FixOM_Electricity_CH4_GT")
        * 1e-2
    )  # percent -> 0...1

    lifetime = get_parameter_values(
        scalars, "EnergyConversion_LifeTime_Electricity_CH4_GT"
    )
...
def update_hydro_reservoir(component_df, scalars):
    ...
    component_df["capacity_turbine"] = get_parameter_values(
        scalars, "EnergyConversion_Capacity_Electricity_Hydro_ReservoirTurbine"
    )

    component_df["capacity_pump"] = get_parameter_values(
        scalars, "EnergyConversion_Capacity_Electricity_Hydro_ReservoirPump"
    )

    storage_capacity = get_parameter_values(
        scalars, "EnergyConversion_Capacity_Electricity_Hydro_ReservoirStorage"
    )

    initial_storage_level = get_parameter_values(
        scalars, "Energy_PrimaryEnergy_Hydro_Reservoir_FillingLevelStart"
    )
    ...
    component_df["efficiency_turbine"] = (
        get_parameter_values(
            scalars, "EnergyConversion_Eta_Electricity_Hydro_ReservoirTurbine"
        )
        * 1e-2
    )  # percent -> 0...1

    component_df["efficiency_pump"] = (
        get_parameter_values(
            scalars, "EnergyConversion_Eta_Electricity_Hydro_ReservoirPump"
        )
        * 1e-2
    )  # percent -> 0...1
    ...
    component_df["marginal_cost"] = (
        get_parameter_values(
            scalars, "EnergyConversion_VarOM_Electricity_Hydro_Reservoir"
        )
        * 1e-3
    )  # Eur/GWh -> Eur/MWh
...
def update_electricity_bev(component_df, scalars):
    ...
    component_df["capacity"] = get_parameter_values(
        scalars, "Transport_CarNumber_Electricity_Cars"
    ) * get_parameter_values(scalars, "Transport_ConnecPower_Electricity_Cars")

    component_df["storage_capacity"] = (
        get_parameter_values(scalars, "Transport_CarNumber_Electricity_Cars")
        * get_parameter_values(scalars, "Transport_BatteryCap_Electricity_Cars")
        * 1e3
    )  # GWh to MWh

    component_df["efficiency_v2g"] = (
        get_parameter_values(scalars, "Transport_EtaFeedIn_Electricity_Cars") * 1e-2
    )  # percentage to decimal

    component_df["amount"] = (
        get_parameter_values(scalars, "Transport_AnnualDemand_Electricity_Cars") * 1e3
    )  # GWh to MWh

    component_df["marginal_cost"] = (
        get_parameter_values(scalars, "Transport_VarOMGridFeedIn_Electricity_Cars")
        * 1e-3
    )  # Eur/GWh to Eur/MWh
...
def update_scalars(select_components, destination, scalars):
    for component, kwargs in select_components.items():
        ...
        component_df_path = os.path.join(destination, "elements", component + ".csv")
